  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 mt-2">需要附加條件核准的案件</p>
      </div>
      <button
        @click="addMockLead"
...
const addMockLead = async () => {
  ...
  if (config.public.apiBaseUrl === '/mock-api') {
    ...
    // 如果是待處理狀態，添加到本地顯示列表
    if (newAddedLead.case_status === 'pending') {
      leads.value.push(newAddedLead)
    }
    // 刷新側邊欄計數
    await refreshBadges()
...
// Call addMockLead 3 times on mount to add initial mock data
onMounted(async () => {
  for (let i = 0; i < 3; i++) {
    await addMockLead();
  }
