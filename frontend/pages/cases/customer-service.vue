<template>
  <div class="space-y-6">
    ...
    <DataTable
      ...
      :columns="customerServiceTableColumns"
      :data="filteredLeads"
      :loading="loading"
      :error="loadError"
      ...
      :loading-text="pageConfig.loadingText"
...
// 引入統一的案件管理配置
const { getPageConfig, getTableColumnsForPage } = useCaseManagement()

// 當前頁面配置 (客服 -> 'customer_service')
const pageConfig = computed(() => getPageConfig('customer_service'))

// 表格配置
const customerServiceTableColumns = computed(() => getTableColumnsForPage('customer_service'))
...
// 由於使用 useCaseManagement，不再需要 HIDDEN_FIELDS_CONFIG
...
// 表格配置
const leadTableColumns = computed(() => customerServiceTableColumns.value)
...
const loadLeads = async () => {
  ...
  try {
    ...
    if (config.public.apiBaseUrl === '/mock-api') {
      ...
      // 過濾出待處理狀態的案件
      leads.value = mockDataStore.leads.filter(lead => lead.case_status === pageConfig.value.caseStatus)
      loading.value = false
      // 確保 badges 刷新
      await refreshBadges()
      return
    ...
    const { items, meta, success: ok } = await listCases({
      case_status: pageConfig.value.caseStatus,
