  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 mt-2">管理已驗證的有效客戶</p>
      </div>
    </div>
...
// 引入統一的案件管理配置
const { PAGE_CONFIGS, getPageConfig, getTableColumnsForPage, ADD_LEAD_FORM_CONFIG, CASE_STATUS_OPTIONS, CHANNEL_OPTIONS, WEBSITE_OPTIONS, PURPOSE_OPTIONS } = useCaseManagement()

// 當前頁面配置 (有效客 -> 'valid_customer')
const pageConfig = computed(() => getPageConfig('valid_customer'))
...
// 從 useCaseManagement 引入 HIDDEN_FIELDS_CONFIG
const { HIDDEN_FIELDS_CONFIG } = useCaseManagement()
...
// 表格配置
const leadTableColumns = computed(() => validCustomerTableColumns.value)
...
const loadLeads = async () => {
  ...
  try {
    ...
    if (config.public.apiBaseUrl === '/mock-api') {
      ...
      // 過濾出有效客狀態的案件
      leads.value = mockDataStore.leads.filter(lead => lead.case_status === 'valid_customer')
      loading.value = false
      // 確保 badges 刷新
      await refreshBadges()
      return
...
const getStatusRouteMapping = (status) => {
  const statusOption = CASE_STATUS_OPTIONS.find(opt => opt.value === status)
